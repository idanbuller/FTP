...
def printProgress(fSize, sentVol):
    print(f"transport Progress {sentVol * (100 / fSize)}%" + " " * 30,
          end="\r")  # calc % of data sent from total file size


def servClient(client, address):
    file_name = [f for f in os.listdir() if f != "extreme_ftp.py" and f != "run.bat"][
        0]  # from list files in dir - look for the file to transfer
    if os.path.isfile(file_name):  # make shure it's a file...
        file_size = os.path.getsize(file_name)  # get the size of the file
        client.send(
            str(file_name + " " + str(file_size)).encode())  # send the client a message with file name & file size
...
def Main():
    server = False if len([f for f in os.listdir('.') if
                           os.path.isfile(f)]) == 2 else True  # must have a script & run.bat file in this dir
